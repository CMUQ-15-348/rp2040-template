...
/* How many microseconds have passed since the processor booted up? */
pub fn get_current_time() -> u64 {
    let lower = read_reg(TIMER_BASE + TIMELR);
    let upper = read_reg(TIMER_BASE + TIMEHR);
    ((upper as u64) << 32) | lower as u64
}

/* Delay for time milliseconds
 * This is a busy wait loop.  It is not very accurate, but it is simple.
 * Don't use this if you need accurate timing.  If you need accurate timing,
 * investigate the RTC peripheral.
 */
pub fn delay(time: u32) {
    let start = get_current_time();
    let mut cur_time = start;
    while start + (time as u64) * 1000 > cur_time {
